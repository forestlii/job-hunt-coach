...
一个面向 Claude（Claude Code / Claude.ai）的求职教练 **Agent Skill**。基于"**面试是博弈，不是证明自己**"的方法论，覆盖求职全链条：简历优化、自我介绍、高频难题应对、gap 与转行叙事、薪资谈判、面试后复盘、多 offer 取舍。
...
- 把 gap 空窗期和转行讲成优势
- 薪资谈判（调研定区间、不先报价、三种报价逻辑、24h 缓冲、风险溢价拿现金）
- 模拟面试、陪练追问
- 面试后复盘、读懂面试信号、等结果阶段的博弈（别钓结果、多机会并行）
- 拿到多个 offer 时该接哪个（职业资本、目标对齐、风险溢价、过渡桥）
...
└── references/
    ...
    ├── self-introduction.md    # 自我介绍（锚定法则、行为带出判断、潜力 vs 已验证）
    ├── tough-questions.md      # 高频难题（缺点 / 离职 / 适配）
    ├── gap-and-pivot.md        # gap 与转行叙事
    ├── salary-negotiation.md   # 薪资谈判（含风险溢价、总包构成）
    ├── post-interview.md       # 面试后复盘、读信号、等结果与多机会博弈
    └── which-offer-to-take.md  # 多 offer 取舍（职业资本 / 目标对齐 / 风险溢价 / 过渡桥）
```
